...
void remove_nested_orf(int *orf_start, int *orf_finish, int orf_count)
{
int i;
const int TEMP_ORFS = 10;
int frame_1_orf[TEMP_ORFS][2];
int frame_2_orf[TEMP_ORFS][2];
int frame_3_orf[TEMP_ORFS][2];
int frame_1_count = 0;
int frame_2_count = 0;
int frame_3_count = 0;
...
for(i = 0; i < orf_count; i++)
	...
	if(orf_start[1][i] == 1)
		{
		frame_1_orf[frame_1_count][0] = orf_start[0][i];
		frame_1_orf[frame_1_count][1] = orf_finish[i];
		frame_1_count++;
		}
	if(orf_start[1][i] == 2)
		{
		frame_2_orf[frame_2_count][0] = orf_start[0][i];
		frame_2_orf[frame_2_count][1] = orf_finish[i];
		frame_2_count++;
		}
	if(orf_start[1][i] == 3)
		{
		frame_3_orf[frame_3_count][0] = orf_start[0][i];
		frame_3_orf[frame_3_count][1] = orf_finish[i];
		frame_3_count++;
		}
	}

for(i = 0; i < frame_1_count; i++)
	{
